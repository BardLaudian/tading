...
def create_model(input_shape):
    ...
    # Primera capa LSTM con regularización L1 y L2, y habilitando la salida a la siguiente capa
    model.add(LSTM(100, input_shape=input_shape, return_sequences=True, kernel_regularizer=l1_l2(l1=1e-5, l2=1e-4)))
    model.add(Dropout(0.2))  # Dropout para reducir el sobreajuste
    
    # Segunda capa LSTM
    model.add(LSTM(50, return_sequences=False))  # No es necesario return_sequences=False, pero se deja para claridad
    model.add(Dropout(0.2))  # Dropout adicional
    
    # Capa densa adicional con función de activación ReLU
    model.add(Dense(50, activation='relu'))
    
    # Capa de salida
    model.add(Dense(1))
    
    # Compilación del modelo con un optimizador Adam y una tasa de aprendizaje personalizada
